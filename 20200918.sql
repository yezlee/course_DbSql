...
주로 사용하는 데이터 타입
1. 숫자 : NUMBER(p,s) p : 전체자리수, s : 소수점 자리수라고 생각하자(더 깊게 들어가면 엄청 어려워)
...
2. 문자 : VARCHAR2(사이즈 - byte), CHAR(X 사실상 안씀) 
        ...
        - VARCHAR2(2000) : 최대 2000 byte를 담을 수 있는 문자 타입
         ...
         CHAR(5) 'test'
         test는 4byte이고 char(5)는 5byte이기 때문에
...
3. 날짜 : DATE - 7byte고정
         ...
         (1바이트가 차이가 남. 그래서 용량이 적은 데이트를 쓰지)
...
제약조건 : 데이터에 이상한 값이 들어가지 않도록 강제하는 설정
     EX)  emp테이블에 empno컬럼에 값이 없는 상태로 들어가는 것을 방지
          emp테이블에 deptno컬럼의 값이 dept테이블에 존재하지 않는 데이터를 입력하는것을 방지
          emp테이블에 empno(사번)컬럼의 값이 중복되지 않도록 방지
          
제약조건은 4가지가 존재, 그중에 한가지의 일부를 별도의 키워드로 제공 (그중 한가지 제약 조건은 2개로 문법을 제공)
ORACLE에서 만들 수 있는 제약조건이 5가지

1. NOT NULL : 컬럼에 반드시 값이 들어게가 하는 제약조건 - 5번에 사실상 포함되는 사항임
2. UNIQUE : 해당 컬럼에 중복된 값이 들어오는 것을 방지하는 제약조건
3. PRIMARY KEY : UNIQUE + NOT NULL 반드시 존재를 해야함
4. FOREIGN KEY : 해당 컬럼이 참조하는 다른 테이블의 컬럼에 값이 존재하는 제약조건
...
제약조건을 생성하는 방법 3가지
1. 테이블을 생성하면서 컬럼 레벨에 제약조건을 생성 - 이게 제일 간단. 그만큼 제약도 많음
        ==> 제약조건을 결합 컬럼(여러개의 컬럼을 합쳐서)에는 적용 불가
2. 테이블을 생성하면서 테이블 레벨에 제약조건을 생성 - 이걸 제일 많이 사용 - 1번을 커버할수 있는게 2번.
...
INSERT INTO dept_test VALUES (90, 'ddit', 'daejeon');
    에러뜸 -> unique constraint (YEZ.SYS_C007091) violated
        이유 : 90번 부서가 이미 존재하는 상태였기 때문에 PRIMARY KEY 제약에 의해 정상적으로 입력될 수 없다.
        
(YEZ.SYS_C007091) - 내가 생성한 제약조건 이름        
...
dept_test 테이블을 생성
컬럼 : dept 테이블과 동일하게
       deptno NUMBER(2), dname VARCHAT2(14), loc VARCHAR(13)
...
CREATE TABLE dept_test(
    deptno NUMBER(2) CONSTRAINT 제약조건 PRIMARY KEY,
    dname VARCHAR2(14),
    loc VARCHAR2(13));


명명규칙 : PK_테이블명 or PK_테이블명_컬럼
...
->부서번호는 같아도 dname 컬럼의 값이 달라서 프라이머리키 설정에 따라 바뀐다.

INSERT INTO dept_test VALUES (90, 'ddit2', 'daejeon');  - 당연히 얘는 안되지. 완전히 값이 똑같으니까
...
INSERT INTO dept_test VALUES (91, NULL, '대전');
    -> 에러뜸 cannot insert NULL into ("YEZ"."DEPT_TEST"."DNAME")
...
INSERT INTO dept_test VALUES (90, 'ddit', 'daejeon');
INSERT INTO dept_test VALUES (90, NULL, 'daejeon'); //널값이 있는애는 중복이되고
INSERT INTO dept_test VALUES (90, NULL, 'daejeon');

INSERT INTO dept_test VALUES (90, 'ddit', 'daejeon');

프라이머리는 널값도 제약을 걸어버리고
유니크는 널값은 오키
          
          


FOREIGN KEY 제약조건 : 참조하는 테이블에 데이터만 입력 가능하도록 제어
다른 제약조건과 다르게 두개의 테이블 간의 제약 조건 설정
...
INSERT INTO emp_test VALUES (9001, 'sally', 10);
    -> 이건 에러남 integrity constraint (YEZ.SYS_C007098) violated - parent key not found
    
    
테이블 레벨 참조 무결성 제약 조건 생성    
DROP TABLE emp_test;
FK_소스테이블_참조테이블 (제약조건명)
CREATE TABLE emp_test (
    empno NUMBER(4),
    ename VARCHAR(10),
    deptno NUMBER(2), 
...
INSERT INTO emp_test VALUES (9000, 'brown', 90);
INSERT INTO emp_test VALUES (9001, 'sally', 10);    
    -> 오류뜸 integrity constraint (YEZ.FK_EMP_TEST_DEPT_TEST) violated - parent key not found
...
DELETE dept_test
WHERE deptno = 90;
-> 에러뜸  integrity constraint (YEZ.FK_EMP_TEST_DEPT_TEST) violated - child record found
-> 이게 지금 자식이 있어서 부모를 못 없애는걸 하는거

참조 무결정 조건 옵션
1. default : 자식이 있는 부모 데이터를 삭제 할 수없다.
2. 참조 무결성 생성시 OPTION - ON DELETE SET NULL
            삭제시 참조하고 있는 자식테이블의 컬럼을 NULL로 만든다.
2. 참조 무결성 생성시 OPTION - ON DELETE CASCADE
...
INSERT INTO emp_test VALUES (9000, 'brown', -5, 'M');    sal체크 -> 실패 마이너스5
INSERT INTO emp_test VALUES (9000, 'brown', 100, 'T');   성별체크 -> 실패. F or M이라서
INSERT INTO emp_test VALUES (9000, 'brown', 100, 'M');   체크통과 -> 성공
...
CREATE TABLE emp_test (
    ...
    gender VARCHAR(1) CONSTRAINT  c_gender CHECK (gender IN ('M', 'F'))
    );    
...
2. DDL은 AUTO COMMIT이 되어버림..... 내가 커밋안해도 저절로. 그래서 위험함.
DROP TABLE emp_test;
DROP TABLE dept_test;
...
위에 인서트하고 아래 크리에잇해주면 저절로 커밋이 됨. 자동으로
그래서 조심해서 해야함. 
...
SELECT 결과를 이용하려 테이블 생성하기


CREATE TABLE AS ==> CTAS 라고 부름
...
2. 기존에 존재하는 컬럼의 변경( 이름, 데이터 타입)
    ** 데이터 타입의 경우는 이미 데이터가 존재하면 수정이 거의 불가능하다고 보면 됨.
       동일한 데이터 타입으로 사이즈를 늘리는 경우는 상관 없음
       ==> 설계시 고려를 충분히 잘 하자
3. 테이블이 이미 생성된 시점에서 제약조건을 추가

테이블 변경으로 못하는 사항
1. 컬럼 순서 못바꿈. 중간에 끼워넣기 안되고 마지막에 추가됨.
...
*** 데이터 타입을 바꾸는 것은 데이터가 없을 떄는 상관 없지만
    데이터가 있을 경우는 사이즈를 늘리는 것을 제외하고는 불가능하다.
...
3. 테이블이 이미 생성된 시점에서 제약조건을 추가, 삭제하기 - 이건 좀 중요
문법
